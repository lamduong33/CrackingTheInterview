...
struct Graph
{
    ...
    bool recursiveDFS(Node* root, std::string value,
...
struct TreeNode
{
    ...
    explicit TreeNode(int t_data) : data{t_data}, left{nullptr}, right{nullptr}
    {}
...
/**
 * Heap is an abstract class to be encompassed by MinHeap and MaxHeap classes. */
class Heap
{
    TreeNode* m_root; // Tree node of the heap
    std::vector<int> m_list; // array representation
public:

    Heap(std::vector<int>& t_list);
